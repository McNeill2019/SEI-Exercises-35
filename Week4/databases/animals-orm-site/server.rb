...
# has_many: spotters - This is establishing a one-to-many relationship, i.e. 'one animal has many spotters' also known as a has-many/belongs-to relationship.It's telling ActiveRecord that there is a connection between these two tables - Animal and Spotter. Specifically, that the spotters table will have an animal_id column in it, which will tell you which animal any specific spotter is interested in.

#so for example, to find the animal that the spotter with id 20 is spotting you would type:  Spotter.find(20).animal

#To find how many spotters an animal has with an id of 11 you would type: Animal.find(11).spotters to get an array of spotters that are spotting the animal with id 11
